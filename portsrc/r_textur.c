...
void R_DrawCollumnInPost(post_t* patch, post_t* cache, int originy)
{
	...
	while (patch->topdelta != 255)
	{
		...
		patch = (post_t*)(((byte*)patch) + patch->length + 2);
	}
}
...
void R_GenerateTexture(maptexture_t* texture)
{
	...
	Z_CacheMalloc(texturezone, size, (void**)&texture->collumndirectory);
	directory = texture->collumndirectory;
	texture->collumndirectory = directory;

	if (custom != 0)
	{
		custom_p = (byte*)(directory + texture->width);
		for (x = 0; x < texture->width; x++)
		{
			if (patchcount[x] > 1)
			{
	...
	for (patch = texture->patches, i = 0; i < texture->patchcount; i++, patch++)
	{
		...
		for (; x < x2; x++, pcol++)
		{
			...
			if (patchcount[x] == 1)
				...
				R_DrawCollumnInPost((post_t*)patchcol, directory[x], patch->originy);
		}
	}
}
